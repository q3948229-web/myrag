...
def generate_sql_from_md(md_path, sql_path):
    ...
    for line in lines:
        if line.startswith("## "):
            ...
            current_h3 = ""
            current_h4 = ""
            save_node(current_h2, 2, "heading", "", "")
            ...
            current_h4 = ""
            save_node(current_h3, 3, "heading", "", "")
        elif line.startswith("#### "):
            if buffer:
                save_node("", 4, "text", "", "\n".join(buffer))
                buffer = []
            ...
            save_node(current_h4, 4, "heading", number, "")
        else:
            if line.strip():
                buffer.append(line)
    ...
    with open(sql_path, "w", encoding="utf-8") as f:
        f.write("CREATE TABLE IF NOT EXISTS handbook_nodes (\n")
        f.write("    id INTEGER PRIMARY KEY,\n")
        f.write("    level INTEGER,\n")
        f.write("    node_type TEXT,\n")
        f.write("    title TEXT,\n")
        f.write("    number TEXT,\n")
    ...
    print(f"SQL 生成完成！已保存至 {sql_path}")
...
def main():
    ...
    print(f"开始并行处理，共 {len(chunks)} 个块...")
    
    results = [None] * len(chunks)
    
    def process_and_store(idx):
        chunk_text = "".join(chunks[idx])
        print(f"正在启动块 {idx+1}/{len(chunks)}...")
        refined = ai_process_chunk(chunk_text)
        results[idx] = refined
        print(f"块 {idx+1} 处理完成。")

    # 使用线程池并发请求 (限制并发数为 5, 避免接口限制)
    ...
    print(f"Markdown 清洗完成！已保存至 {OUTPUT_MD}")
    
    # 2. 从清洗后的 Markdown 生成 SQL
